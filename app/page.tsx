...
const heroImage = "https://images.unsplash.com/photo-1600566753190-17f0baa2a6c3?auto=format&fit=crop&w=2400&q=90";
    ...
    <main className="overflow-x-clip bg-[var(--paper)] text-[var(--forest)]">
      <section className="relative isolate overflow-hidden bg-[var(--paper)] md:min-h-[68svh]">
        <div className="relative h-[43svh] min-h-76 md:absolute md:inset-0 md:h-auto md:min-h-0">
          ...
          <div className="absolute inset-0 hidden bg-[linear-gradient(90deg,rgba(16,44,37,.24),transparent_55%)] md:block" />
        </div>
        ...
        <header className="relative z-20 bg-[var(--paper)]/95 backdrop-blur-sm md:bg-[var(--paper)]/82">
          <div className="mx-auto flex h-18 max-w-[1800px] items-center justify-between px-5 sm:px-8 lg:h-21 lg:px-12 xl:px-16">
            ...
            <nav aria-label="Primary navigation" className="hidden items-center gap-8 text-[10px] font-bold uppercase tracking-[0.16em] lg:flex">
            ...
            <button type="button" aria-controls="mobile-navigation" aria-expanded={menuOpen} aria-label={menuOpen ? "Close menu" : "Open menu"} onClick={() => setMenuOpen((isOpen) => !isOpen)} className="grid h-10 w-10 place-items-center border border-[var(--forest)]/25 lg:hidden"><Icon name={menuOpen ? "close" : "menu"} /></button>
            <Link href="/contact" className="hidden border-b border-current pb-1 text-[10px] font-bold uppercase tracking-[0.15em] lg:block">Plan your space</Link>
          </div>
        </header>
      ...
      {menuOpen && <div id="mobile-navigation" className="fixed inset-0 z-50 bg-[var(--forest-deep)] px-6 py-6 text-[var(--cream)] lg:hidden">
      ...
      <section id="categories" className="bg-[var(--paper)] text-[var(--forest)]">
        <div className="flex flex-col items-center gap-4 px-6 pb-10 pt-18 text-center sm:pt-22 lg:pb-14"><div><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--moss)]">Explore by need</p><h2 className="mt-3 font-[family-name:var(--font-display)] text-4xl leading-none tracking-[-0.04em] sm:text-5xl">Shape your outdoors.</h2></div><Link href="/shop" className="text-[10px] font-bold uppercase tracking-[0.16em] text-[var(--forest)]/70 hover:text-[var(--forest)] sm:inline-flex">View all products <Icon name="arrow" className="ml-2 h-4 w-4" /></Link></div>
        <div className="flex snap-x snap-mandatory gap-4 overflow-x-auto bg-[var(--paper)] px-5 pb-16 [scrollbar-width:none] sm:px-10 lg:grid lg:grid-cols-4 lg:gap-5 lg:px-14 lg:pb-22 xl:px-16">
          {productCategories.map((category, index) => {
            ...
            const image = categoryData?.coverImage || fallbackProduct?.images?.[0] || heroImage;
            return <Link key={category.id} href={`/shop?category=${category.id}`} className="group block w-[78vw] shrink-0 snap-start lg:w-auto">
              <div className="relative aspect-[4/3] overflow-hidden rounded-t-md bg-[var(--sand)] lg:aspect-square"><Image src={image} alt={category.title} fill sizes="(min-width: 1024px) 25vw, 78vw" className={`homepage-image object-cover ${index === 1 ? "object-[60%_center]" : "object-center"}`} /></div>
              <div className="px-1 pt-4 text-center"><p className="text-[9px] font-semibold uppercase tracking-[0.16em] text-[var(--moss)]">0{index + 1}</p><h3 className="mt-2 font-[family-name:var(--font-display)] text-2xl leading-tight sm:text-3xl">{category.title}</h3><p className="mx-auto mt-2 max-w-xs text-sm leading-5 text-[var(--ink)]/65">{category.description}</p><span className="mt-4 inline-flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.14em]">Discover <Icon name="arrow" className="h-4 w-4 transition-transform group-hover:translate-x-1" /></span></div>
            </Link>;
          })}
        </div>
      </section>

      <section className="homepage-paper px-6 py-18 sm:px-10 sm:py-24 lg:px-14 lg:py-30 xl:px-16"><div className="mx-auto max-w-[1800px]"><div className="mb-11 flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--moss)]">Selected for the season</p><h2 className="mt-4 font-[family-name:var(--font-display)] text-5xl leading-none tracking-[-0.045em] sm:text-6xl">The outdoor edit.</h2></div><Link href="/shop" className="text-[10px] font-bold uppercase tracking-[0.16em] hover:text-[var(--clay)]">Shop the full collection <Icon name="arrow" className="ml-2 inline h-4 w-4" /></Link></div>
        {featuredProducts.length ? <div className="grid gap-x-5 gap-y-11 sm:grid-cols-2 lg:grid-cols-4">{featuredProducts.map((product) => <Link key={product.id} href={`/products/${product.id}`} className="group"><div className="relative aspect-[4/3] overflow-hidden bg-[var(--sand)]"><Image src={product.images[0] || heroImage} alt={product.titleEn} fill sizes="(min-width: 1024px) 25vw, (min-width: 640px) 50vw, 100vw" className="homepage-image object-cover" /></div><p className="mt-4 text-[9px] font-bold uppercase tracking-[0.16em] text-[var(--moss)]">{productCategories.find((category) => category.id === product.category)?.title || product.category}</p><div className="mt-2 flex items-start justify-between gap-3"><h3 className="font-[family-name:var(--font-display)] text-xl leading-tight">{product.titleEn}</h3><span className="shrink-0 text-sm">${product.price.toLocaleString()}</span></div></Link>)}</div> : <p className="border-y border-[var(--forest)]/15 py-12 text-sm text-[var(--ink)]/65">Our product selection is being prepared. Please check back soon.</p>}</div></section>
