...
await prisma.user.upsert({ where: { email: email.toLowerCase() }, update: { passwordHash, name, role: "OWNER", status: "ACTIVE" }, create: { email: email.toLowerCase(), passwordHash, name, role: "OWNER" } });
