...
商品兴趣分析记录匿名访客 ID、原始 IP、User-Agent、来源页面、商品和访问时间。后台同时提供商品访问量/访客数排行与最近访客明细；客户提交询盘时，通过同一访客 ID 展示其此前浏览过的商品。相同访客在 30 分钟内重复访问同一商品只计一次，减少刷新造成的数据噪声。隐私政策必须如实说明该用途。部署在 Nginx 后必须仅信任本机反向代理传来的 `X-Forwarded-For`，不能直接信任任意客户端提供的 IP 头。
...
### 19.6 当前实现状态（2026-09-04）

已完成：本机 PostgreSQL 数据库与 Prisma 表、数据库用户会话、bcrypt 密码校验、`HttpOnly` 会话 Cookie、所有现有 `/api/admin/*` 路由的服务端鉴权、Owner 新增/停用 Editor、密码重置与会话撤销、Owner 访问 IP/设备和操作审计页面。客户联系表单会写入 `Inquiry`，并记录 IP、设备及由 IP 解析的国家、省/州、城市；后台可查看、更新状态和记录内部备注。商品详情访问写入 `PageView`，后台显示热门商品、独立访客、地区汇总、最近访问 IP/地区/来源，并将询盘关联到该访客此前浏览过的商品。地区解析只对公网 IP 执行，本地网络地址会显示为未知。旧的 `NEXT_PUBLIC_ADMIN_PASSWORD` 和浏览器 `sessionStorage` 登录已移除。当前前台实际使用的 `data/database.json` 已幂等复制到 PostgreSQL；公开商品 API、后台商品 CRUD 和分类管理都已切换至 PostgreSQL，保留现有商品 URL ID、中英文标题、图片、特点、规格及分类封面。`data/database.json` 只作为可恢复的旧数据快照保留。
